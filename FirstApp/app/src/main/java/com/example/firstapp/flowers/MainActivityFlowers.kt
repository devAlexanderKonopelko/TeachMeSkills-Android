...
import kotlinx.android.synthetic.main.activity_main_flower.*
...
class MainActivityFlowers : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            addFlower.id -> {
                val newIntent = Intent(this, com.example.firstapp.flowers.AddFlowerActivity::class.java)
            ...
            addBottomFragment.id -> {
                if (fragmentBottom.getFlower() != null) {
                    supportFragmentManager.beginTransaction().replace(bottomFragment.id, fragmentBottom).commit()
                } else {
                    val newToast = Toast.makeText(
                        this,
                        "Flower not found. Please, add a flower",
